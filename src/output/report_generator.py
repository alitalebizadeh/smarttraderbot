...
<div class="section setup-card {'bull' if direction == 'bullish' else 'bear'}">
  ...
  <p>زون کامل: <span class="price">{zbottom:.2f}</span> — <span class="price">{ztop:.2f}</span><br>میانه زون: <span class="price">{midpoint:.2f}</span><br>حد ضرر: <span class="price">{stop_val:.2f}</span> (بافر ۱۰٪ زون)<br>ناحیه P/D: {getattr(cluster, 'premium_discount_zone', 'unknown')}</p>
